...
class InformacaoAtividades extends StatelessWidget {
  NavigationController _navigationController= locator<NavigationController>();
  final Activity showingActivity;

  InformacaoAtividades({Key key, this.showingActivity}) : super(key: key);
            ...
            child: Column(
              ...
              children: <Widget> [
                ...
                Row(
                  ...
                  children: [
                    Visibility(
                      visible: (model.currentUser != null && showingActivity.creator== model.currentUser.id) ? true : false,
                      child: BusyButton(
                        title: 'Deletar',
                        onPressed: () async {
                          await model.confirmDelete();
                          if (model.isDeleteAccepted) {
                            model.deleteActivity(showingActivity);
                          }
                        },
                      ),
                    ),
                    horizontalSpaceTiny,
                    if (model.isUserInActivity()!= null && !model.isUserInActivity())
                    BusyButton(
                      title: 'Entrar',
                      onPressed: (){
                        if (model.currentUser== null){
                          ...
                          model.insertUser(showingActivity, model.currentUser);
                          // _navigationController.navigateTo(MyActivityPageRoute);
                        }
//                        _navigationController.navigateTo(MyActivitiesPageRoute);
                      },
                    ),
                    if (model.isUserInActivity()!= null && model.isUserInActivity())
                      BusyButton(
